...
export async function fetch_programming_exercises_by_courseId(
  ...
  const data = await artemisRequestJson<any>(`/api/core/courses/${courseId}/for-dashboard`, { token });
...
export async function fetch_exercise_by_id(
  ...
  const data = await artemisRequestJson<{ [key: string]: any }>(`/api/exercise/exercises/${exerciseId}`, { token });
